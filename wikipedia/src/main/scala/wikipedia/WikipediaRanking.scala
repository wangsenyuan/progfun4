...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
        val a = rdd.filter(_.text.split(" ").exists(_ == (lang)))
        a.aggregate(0)((x, _) => x  + 1, _ + _)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =  {  
     val a = rdd.flatMap(w => {
      val words = w.text.split(" ").toSet
      langs.filter(lang => words(lang)).map(lang => lang -> w)
    })
    a.groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val a = rdd.flatMap(w => {
      val words = w.text.split(" ").toSet
      langs.filter(words).map(lang => lang -> 1)
    })
    a.reduceByKey(_ + _).collect().toList.sortBy(_._2).reverse
  }
